...
import urllib.parse
import argparse
import traceback # ИМПОРТИРОВАНО для подробных логов ошибок
from concurrent.futures import ThreadPoolExecutor, as_completed
import requests
from tqdm import tqdm

# --- НОВЫЙ БЛОК: НАСТРОЙКА ЛОГИРОВАНИЯ ---
# Включаем режим отладки, если установлена переменная окружения DEBUG=true
# В GitHub Actions это управляется из yml-файла
DEBUG = os.environ.get('DEBUG', 'false').lower() == 'true'

def log_debug(message):
    """Выводит сообщение, только если включен режим отладки."""
    if DEBUG:
...
XRAY_PATH = "./xray"
MAX_WORKERS = 100
REQUEST_TIMEOUT = 12  # ИЗМЕНЕНО: Увеличен тайм-аут для надежности в Actions
TARGETS = {
...
# --- ФУНКЦИИ ПАРСИНГА И ГЕНЕРАЦИИ (с мелкими правками для надежности) ---
def parse_proxy_link(link):
    try:
        ...
        if not protocol or not parsed_url.hostname or not parsed_url.port:
            log_debug(f"[PARSE_FAIL] Неполная ссылка: {link[:50]}...")
            return None
        ...
        log_debug(f"[PARSE_ERROR] Ошибка парсинга '{link[:50]}...': {e}")
        return None

def generate_xray_config(proxy_data, local_port):
    try:
        protocol = proxy_data['protocol']
        config = {"log": {"loglevel": "warning"},"inbounds": [{"port": local_port,"listen": "127.0.0.1","protocol": "socks","settings": {"auth": "noauth", "udp": True, "ip": "127.0.0.1"}}],"outbounds": [{"protocol": protocol, "settings": {}, "streamSettings": {}}]}
...
# --- ОСНОВНАЯ ФУНКЦИЯ ПРОВЕРКИ (значительно переработана для логирования) ---
def check_proxy(proxy_link):
    ...
    try:
        ...
        # Запускаем Xray, захватывая stderr для отладки
        proc = subprocess.Popen([XRAY_PATH, "run", "-c", config_path], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, encoding='utf-8')
        time.sleep(1.5)
        
        # Проверяем, не "упал" ли Xray сразу после запуска
        if proc.poll() is not None:
            log_debug(f"-> Xray процесс завершился преждевременно с кодом {proc.poll()}.")
            stderr_output = proc.stderr.read()
            log_debug(f"-> Xray stderr: {stderr_output.strip()}")
            return None
        
        proxies = {'http': f'socks5://127.0.0.1:{local_port}', 'https': f'socks5://127.0.0.1:{local_port}'}
        
        # 1. Проверка пинга (доступности)
        ...
        log_debug(f"--- НЕПРЕДВИДЕННАЯ ОШИБКА в check_proxy ---")
